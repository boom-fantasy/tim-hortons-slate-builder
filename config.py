...
def _validate_fallback(f: FallbackConfig) -> list[str]:
    ...
    if f.lookback_games > 20:
        problems.append(
            ...
            f"line changes will dominate the average, and each extra game costs an "
            f"API call per team per unpriced game"
        )
    ...
    if f.line_type not in ("opening", "closing"):
        problems.append(
            f"fallback.line_type must be 'opening' or 'closing', got {f.line_type!r}"
        )
    return problems
...
def load(path: str | Path | None = None) -> Config:
    ...
    try:
        ...
        fallback = FallbackConfig(
            ...
            line_type=str(fb.get("line_type", "opening")).lower(),
